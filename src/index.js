...
let lastScrollTop = 0;
let headerHeight = $('div.header-container').outerHeight();
let menuSnippet = "";
let cookieSnippet = '';
...
function createCookie(cookieName, cookieVal, exp) {
    ...
    let expiresIn = "expires=" + cookieDate.toUTCString();
    document.cookie = cookieName + "=" + cookieVal + ";" + expiresIn + ";path=/"; 
}

function checkCookie(cookieName) {
    let checkName = new RegExp('^(.*;)?\\s*' + cookieName + '\\s*=').test(document.cookie);
    return checkName;
}
...
function createBlockOverlay() {
    ...
    let scrollYOffset = document.documentElement.scrollTop || document.body.scrollTop;
    const blockOverlay = document.createElement('div');
    document.getElementById('index-html').prepend(blockOverlay);
...
function removeMenu() {
    // 1. call fn removeBlockOverlay();
    // 2. remove div "menuArea" and children;
    // 3. shift screenDiv right;
}

function menuButton() {
    // 1. add event click event listener with switch
